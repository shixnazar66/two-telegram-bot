...
const KNOWN_TYPES = new Map(deps_node_js_1.GLOBAL_CONSTRUCTOR_MAP);
const e = KNOWN_TYPES.get(Error.name);
KNOWN_TYPES.delete(Error.name);
KNOWN_TYPES.set(deps_node_js_1.GrammyError.name, {
    instance: deps_node_js_1.GrammyError,
    from: (err) => {
        const res = [
            err.message,
            err.error_code,
    ...
    create: ([message, error_code, description, parameters, method, payload, stack, cause,]) => {
        const err = new deps_node_js_1.GrammyError(message, { ok: false, error_code, description, parameters }, method, payload);
        if (stack === undefined)
            delete err.stack;
        else
            err.stack = stack;
...
KNOWN_TYPES.set(deps_node_js_1.HttpError.name, {
    instance: deps_node_js_1.HttpError,
    from: (err) => {
        const res = [err.message, err.error];
        if (err.stack !== undefined)
            res.push(err.stack);
        if (err.cause !== undefined) {
            if (err.stack === undefined)
    ...
    create: ([message, error, stack, cause]) => {
        const err = new deps_node_js_1.HttpError(message, error);
        if (stack === undefined)
            delete err.stack;
        else
            err.stack = stack;
...
class ConversationControls {
    ...
    async active() {
        var _a;
        return Object.fromEntries(Object.entries((_a = (await this[internal].session()).conversation) !== null && _a !== void 0 ? _a : {})
 ...
 */
function conversationRunner(ctx, builder, timeout) {
    /**
    ...
    async function run(data) {
        ...
        const handle = new ConversationHandle(ctx, data, rsr, timeout);
        ...
        return (_a = rsr.value) !== null && _a !== void 0 ? _a : { consumed: true, exit: true };
    }
    return run;
}
/**
...
function conversations() {
    return async (ctx, next) => {
        ...
        (_a = ctx.conversation) !== null && _a !== void 0 ? _a : (ctx.conversation = new ConversationControls(async () => {
            // Access session lazily
            const session = await ctx.session;
            if (!transformed) {
                transformed = true;
                if (typeof session.conversation === "number" ||
                    (typeof session.conversation === "object" &&
                        Array.isArray(session.conversation))) {
                    session.conversation = (0, deps_node_js_1.delistify)(session.conversation, KNOWN_TYPES);
                }
            }
            return session;
        }));
 ...
 */
function createConversation(builder, config = {}) {
    const { id = builder.name, maxMillisecondsToWait } = typeof config === "string" ? { id: config } : config;
    if (!id)
    ...
    return async (ctx, next) => {
        ...
        const runOnData = conversationRunner(ctx, builder, maxMillisecondsToWait);
        /**
        ...
        async function runUntilComplete(conversations) {
            let op = { consumed: false, exit: false };
        ...
        ctx.conversation.enter = async (enterId, opts) => {
            ...
            (_a = session.conversation) !== null && _a !== void 0 ? _a : (session.conversation = {});
            const entry = {
                log: { u: [] },
                last: Date.now(),
            };
            ...
            if (opts === null || opts === void 0 ? void 0 : opts.overwrite)
                ...
                ((_b = (_c = session.conversation)[id]) !== null && _b !== void 0 ? _b : (_c[id] = [])).push(...append);
        ...
        try {
            ...
            let op = { consumed: false, exit: false };
            if (((_a = session.conversation) === null || _a === void 0 ? void 0 : _a[id]) !== undefined) {
                try {
...
function toObj(otherwise) {
    return typeof otherwise === "function" ? { otherwise } : otherwise !== null && otherwise !== void 0 ? otherwise : {};
}
/**
 * Internally used class which acts as a conversation handle.
 */
class ConversationHandle {
    constructor(ctx, data, rsr, timeout) {
        this.ctx = ctx;
        this.data = data;
        this.rsr = rsr;
        ...
        this.active = true;
        this.mw = new deps_node_js_1.Composer();
        /**
    ...
    async _replayWait() {
        ...
        if (!this._isReplaying) {
            ...
            ctx = Object.assign(new deps_node_js_1.Context(u, this.ctx.api, this.ctx.me), x);
            // Copy over functions which we could not store
            f.forEach((key) => {
                // deno-lint-ignore no-explicit-any
                const current = this.ctx[key];
                if (typeof current === "function") {
                    // deno-lint-ignore no-explicit-any
                    ctx[key] = current.bind(this.ctx);
                }
            });
        }
        this.currentCtx = ctx;
    ...
    _replayApi(method) {
        var _a, _b, _c, _d;
        var _e;
        let index = (_a = this.replayIndex.api) === null || _a === void 0 ? void 0 : _a.get(method);
        if (index === undefined) {
            index = 0;
            (_b = (_e = this.replayIndex).api) !== null && _b !== void 0 ? _b : (_e.api = new Map());
            this.replayIndex.api.set(method, index);
        }
        const result = (_c = this.data.log.u[this.replayIndex.wait - 1].a) === null || _c === void 0 ? void 0 : _c[method][index];
        (_d = this.replayIndex.api) === null || _d === void 0 ? void 0 : _d.set(method, 1 + index);
        if (result === undefined) {
            return new Promise(() => { });
        }
    ...
    _replayExt() {
        ...
        const result = (_a = this.data.log.u[this.replayIndex.wait - 1].e) === null || _a === void 0 ? void 0 : _a[index];
    ...
    _logApi(method) {
        ...
        const slot = { i: -1 };
        ((_b = (_d = ((_a = (_c = this.data.log.u[index - 1]).a) !== null && _a !== void 0 ? _a : (_c.a = {})))[method]) !== null && _b !== void 0 ? _b : (_d[method] = [])).push(slot);
        return slot;
    }
    /**
    ...
    _logExt() {
        ...
        const index = this.replayIndex.wait;
        const slot = { i: -1 };
        ((_a = (_b = this.data.log.u[index - 1]).e) !== null && _a !== void 0 ? _a : (_b.e = [])).push(slot);
        return slot;
    }
    /**
    ...
    _finalize(slot) {
        var _a;
        var _b;
        slot.i = (_a = (_b = this.replayIndex).resolve) !== null && _a !== void 0 ? _a : (_b.resolve = 0);
    ...
    _resolveAt(index, value) {
        ...
        ((_a = (_b = this.replayIndex).tasks) !== null && _a !== void 0 ? _a : (_b.tasks = []))[index] = r;
        const resolveNext = () => {
            var _a;
            var _b;
            if (this.replayIndex.tasks === undefined)
                return;
            (_a = (_b = this.replayIndex).resolve) !== null && _a !== void 0 ? _a : (_b.resolve = 0);
     ...
     * incoming update.
     */
    async wait(opts = { maxMilliseconds: this.timeout }) {
        ...
        if (this._isReplaying) {
            ...
            if (!this._isReplaying && // limit to the current wait
     ...
     */
    async waitUnless(predicate, opts) {
    ...
    async waitFor(query, opts) {
        return await this.waitUntil(deps_node_js_1.Context.has.filterQuery(query), opts);
    }
    /**
     ...
     * @param opts Optional config for discarded updates
     */
    async waitForHears(trigger, opts) {
        return await this.waitUntil(deps_node_js_1.Context.has.text(trigger), opts);
    }
    /**
     ...
     * @param opts Optional config for discarded updates
     */
    async waitForCommand(command, opts) {
        return await this.waitUntil(deps_node_js_1.Context.has.command(command), opts);
    }
    /**
     ...
     * @param opts Optional config for discarded updates
     */
    async waitForCallbackQuery(trigger, opts) {
        return await this.waitUntil(deps_node_js_1.Context.has.callbackQuery(trigger), opts);
    }
    /**
     ...
     * @param opts Optional config for discarded updates
     */
    async waitForGameQuery(trigger, opts) {
        return await this.waitUntil(deps_node_js_1.Context.has.gameQuery(trigger), opts);
    }
    /**
     * Waits for a new update (e.g. a message, callback query, etc) from the
     ...
     * @param opts Optional config for discarded updates
     */
    async waitFrom(user, opts) {
        const id = typeof user === "number" ? user : user.id;
        const predicate = (ctx) => { var _a; return ((_a = ctx.from) === null || _a === void 0 ? void 0 : _a.id) === id; };
     ...
     * @param opts Optional config for discarded updates
     */
    async waitForReplyTo(message_id, opts) {
        const id = typeof message_id === "number"
            ? message_id
            : message_id.message_id;
        return await this.waitUntil((ctx) => {
            var _a, _b, _c, _d;
    ...
    async run(...middleware) {
        ...
        this.mw.use(async (ctx, next) => {
            if (await runAsLeaf(ctx, ...middleware))
                await next();
        });
    }
}
exports.ConversationHandle = ConversationHandle;
async function runAsLeaf(ctx, ...middleware) {
    const mw = new deps_node_js_1.Composer(...middleware).middleware();
